...
dies.map((elemento, numero) => {
  ejercici3[elemento] = numero;
});

console.log(ejercici3);
...
console.log( musics.map((elemento) => elemento.nom));
...
musics.forEach((elemento) => {
  (elemento.instrument === "guitarra")?guitarristes.push(elemento.nom):"";
});
console.log(guitarristes);
//map
let ejer5=[];
musics.map(element=>{
(element.instrument==="guitarra")?ejer5.push(element.nom):""});
console.log(ejer5);

//Amb filter

function filtro(element) {
   return element.instrument==="guitarra";
}

let git = musics.map((element) => element).filter(filtro);

console.log(git.map((elemento) => elemento.nom))
//Amb reduce
//['Pep', 'Vicent Lloret']
musics.reduce((elemento) => {});
